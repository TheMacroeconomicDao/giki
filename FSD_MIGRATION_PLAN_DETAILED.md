...
- ⬜ Модуль `shared/ui/button` (базовая кнопка)
- ⬜ Модуль `shared/ui/input` (поле ввода)
- ⬜ Модуль `shared/ui/modal` (модальное окно)
- ⬜ Модуль `shared/ui/spinner` (индикатор загрузки)
- ⬜ Модуль `shared/ui/notification` (уведомление)
